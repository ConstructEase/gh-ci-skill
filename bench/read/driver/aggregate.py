#!/usr/bin/env python3
import os
"""Aggregate results.tsv into per-(task x condition) medians and IQRs.
...
def main(path=None):
    ...
    print()
    hdr = f"{'task':5} {'cond':9} {'n':>2} {'succ':>5} {'totIn med (IQR)':>26} {'out':>6} {'wall s':>8} {'api':>4} {'tool':>5}"
    print(hdr)
    print("-" * len(hdr))
    for task in TASKS:
        for cond in CONDS:
            ...
            print(
                f"{task:5} {cond:9} {c['n']:>2} {c['success_rate']*100:>4.0f}% "
    ...
    for cond, c in roll.items():
        ...
        print(
            f"  {cond:9} n={c['n']:>3} success={c['success_rate']*100:>5.1f}% "
